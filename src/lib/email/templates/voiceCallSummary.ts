...
export function buildVoiceCallSummaryEmail(data: VoiceCallSummaryData): {
  ...
  const subject = `[${style.label}] Voice Lead: ${escapeHtml(data.leadName || 'Caller')} — ${formatDuration(data.callDuration)} call`;
...
<div style="font-family:Inter,system-ui,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
  ...
  <div style="background:white;border:1px solid #E5E7EB;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
    ...
    <h2 style="margin:12px 0 8px;color:#1B2A4A;">${escapeHtml(data.leadName || 'Unknown Caller')}</h2>
    <table style="width:100%;border-collapse:collapse;margin:16px 0;">
      <tr><td style="padding:8px 0;color:#6B7280;width:140px;">Phone</td><td style="padding:8px 0;color:#111827;">${escapeHtml(data.leadPhone || 'Not provided')}</td></tr>
      <tr><td style="padding:8px 0;color:#6B7280;">Email</td><td style="padding:8px 0;color:#111827;">${escapeHtml(data.leadEmail || 'Not provided')}</td></tr>
    ...
    <div style="margin:16px 0;">
      <a href="${escapeHtml(data.recordingUrl)}" style="color:#0070F3;text-decoration:none;font-weight:600;">
        Listen to Recording
      </a>
    </div>` : ''}
